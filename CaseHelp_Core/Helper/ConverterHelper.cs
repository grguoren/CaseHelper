...
namespace CaseHelp_Core.Helper
{
    ...
    public static class ConverterHelper
    {
        ...
        public static string TimeSpans(this DateTime date)
        {
            TimeSpan span = DateTime.Now - date;
            string result = "";
            var intDays = Convert.ToInt32(span.TotalDays);
            var intHours = Convert.ToInt32(span.TotalHours);
            var intMinutes = Convert.ToInt32(span.TotalMinutes);

            if (span.TotalDays > 60)
            {
                result = date.ToCnDataString();
            }
            else if (intDays > 0)
            {
